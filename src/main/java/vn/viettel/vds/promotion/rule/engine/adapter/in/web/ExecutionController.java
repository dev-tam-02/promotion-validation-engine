...
import vn.viettel.vds.promotion.rule.engine.adapter.in.web.dto.ExecuteRequest;
import vn.viettel.vds.promotion.rule.engine.adapter.in.web.dto.ExecuteResponse;
...
public class ExecutionController {
    ...
    public ResponseEntity<ExecuteResponse> executeRule(@Valid @RequestBody ExecuteRequest request) {
        ...
        try {
            ...
            // Execute rule
            vn.viettel.vds.promotion.rule.engine.application.dto.ExecuteResponse result = ruleEnginePort.execute(input);
    ...
    public ResponseEntity<List<ExecuteResponse>> executeBatch(
        ...
        try {
            ...
            // Execute batch
            List<vn.viettel.vds.promotion.rule.engine.application.dto.ExecuteResponse> results = ruleEnginePort.executeBatch(inputs);

            // Convert results
            List<ExecuteResponse> responses = new ArrayList<>();
            for (vn.viettel.vds.promotion.rule.engine.application.dto.ExecuteResponse result : results) {
    ...
    private ExecuteResponse mapToExecuteResponse(vn.viettel.vds.promotion.rule.engine.application.dto.ExecuteResponse result) {
        ...
        if (result.getExplain() != null) {
            for (vn.viettel.vds.promotion.rule.engine.application.dto.ExecuteResponse.ExplainEntry entry : result.getExplain()) {
